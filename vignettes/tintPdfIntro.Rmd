...
vignette: >
  %\VignetteIndexEntry{Short tint PDF Example}
...
[tint](http://github.com/eddelbuettel/tint) \citep{cran:tint} updates the look and feel of
'Tufte' documents for R. It combines the (html and pdf parts of the) excellent
[tufte](https://github.com/rstudio/tufte) package \citep{cran:tufte} with the [Roboto
...
One feature of the [tint](http://github.com/eddelbuettel/tint) /
...
\citep{github:ggtufte} (but which conflicts in its font settings with our, so we
...
```{r fig-lato-screenshot, fig.margin=TRUE, fig.width=4, fig.height=4, cache=TRUE, echo=FALSE}
knitr::include_graphics("figures/tintLatoP1.png", dpi = NA)
```
```yaml
latexfonts: 
  - package: lato
...
```{r fig-garamond-screenshot, fig.margin=TRUE, fig.width=4.25, fig.height=5.5, cache=TRUE, echo=FALSE}
knitr::include_graphics("figures/TintGaramondP1.png", dpi = NA)
```
...
square brackets: `[@someone:1984]`. Alternatively, the standard `natbib` features are
available such as `\citet{}`, `\citep{}` and more.
...
\citep{cran:knitr} is driving the conversion, many of its options also apply.  See the
