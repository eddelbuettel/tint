...
If you have any feature requests or find bugs in **tint**, please do not hesitate to file them to https://github.com/eddelbuettel/tint/issues. For general questions, you may ask them on StackOverflow: http://stackoverflow.com/tags/rmarkdown.
...
`r newthought('In his later books')`^[[Beautiful Evidence](http://www.edwardtufte.com/tufte/books_be)], Tufte starts each section with a bit of vertical space, a non-indented paragraph, and sets the first few words of the sentence in small caps. To accomplish this using this style, call the `newthought()` function in **tint** in an _inline R expression_ `` `r ` `` as demonstrated at the beginning of this paragraph.^[Note you should not assume **tint** has been attached to your R session. You should either `library(tint)` in your R Markdown document before you call `newthought()`, or use `tint::newthought()`.]
...
We know from the Markdown syntax that paragraphs that start with `>` are converted to block quotes. If you want to add a right-aligned footer for the quote, you may use the function `quote_footer()` from **tint** in an inline R expression. Here is an example:
...
The rest of this document consists of a few test cases to make sure everything still works well in slightly more complicated scenarios. 

## Two plots in a figure
First we generate two plots in one figure environment with the chunk option `fig.show = 'hold'`. It is possible that the plot is pushed to the page if it is too big. In that case add `out.width="85%"` option to the chunk, or any other percentage that makes the plots fit in the page.
```{r fig-two-together, fig.cap="Two plots in one figure environment.", fig.show='hold', out.width="85%", cache=TRUE, message=FALSE}
p <- ggplot(mtcars2, aes(hp, mpg, color = am)) +
  geom_point()        # First plot
p
p + geom_smooth()     # Second plot
```

## Two plots in separate figures
Then two plots in separate figure environments (the code is identical to the previous code chunk, but the chunk option is the default `fig.show = 'asis'` now):

```{r fig-two-separate, ref.label='fig-two-together', fig.cap=sprintf("Two plots in separate figure environments (the %s plot).", c("first", "second")), cache=TRUE, out.width="80%"}
```

You may have noticed that the two figures have different captions, and that is because we used a character vector of length 2 for the chunk option `fig.cap` (something like `fig.cap = c('first plot', 'second plot')`).

We also reduced the size of the figures canvas with `out.width` to make the two plots fit in the same page.

Another thing we did, to avoid code repetition, is using `ref.label='fig-two-together'` to apply the code in the chunk `fig-two-together`.

## Multiple plots as margin figures
...
## Same chunk plots as different figures
...
knitr::kable(head(iris, 15))      # first table
p
knitr::kable(head(iris, 12))      # second table
p + geom_smooth(method = 'lm')
knitr::kable(head(iris, 5))       # third table
```
...
## Omitting captions in figures
You do not have to assign captions to figures. We show three figures with no captions below **(1)** in the margin, **(1)** in the main column, and **(3)** in full width, respectively.
...
We hope you will enjoy the simplicity of R Markdown and this R package, and we sincerely thank the authors of the Tufte-CSS and Tufte-LaTeX projects for developing the beautiful CSS and LaTeX classes. Our **tint** package would not have been possible without their heavy lifting.
